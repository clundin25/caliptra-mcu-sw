...
const MCU_SRAM_START_ADDR: AxiAddr = AxiAddr {
    lo: 0x0000_0000,
...
const EXTERNAL_SRAM_START_ADDR: AxiAddr = AxiAddr {
    lo: 0x0000_0000,
...
impl DmaPeripheral for DummyDmaCtrl {
    ...
    {
        self.dest_addr_low.reg.set(_val);
    }
    fn read_dma_control(
        &mut self,
    ) -> emulator_bus::ReadWriteRegister<
...
mod test {
    ...
    fn test_helper_setup_dummy_external_sram() -> Rc<RefCell<Ram>> {
        Rc::new(RefCell::new(Ram::new(vec![0u8; EXTERNAL_TEST_SRAM_SIZE as usize])))
    }
    ...
    fn test_helper_setup_autobus(
        ...
        let mut dma_controller = Box::new(
            DummyDmaCtrl::new(clock, dma_ctrl_error_irq, dma_ctrl_event_irq, external_sram).unwrap(),
        );
    ...
    fn test_dma_ctrl_regs_access() {
        ...
        .unwrap();
        assert_eq!(
            bus.read(RvSize::Word, dma_ctrl_base_addr + DMA_INTERRUPT_ENABLE_OFFSET)
                .unwrap(),
            DmaInterruptEnable::Error::SET.value
        );
        ...
        .unwrap();
        assert_eq!(
            bus.read(RvSize::Word, dma_ctrl_base_addr + DMA_INTERRUPT_STATE_OFFSET)
                .unwrap(),
