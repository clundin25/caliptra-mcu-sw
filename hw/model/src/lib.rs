...
impl<'a> InitParams<'a> {
    fn summary(&self) -> InitParamsSummary {
        InitParamsSummary {
            rom_sha384: sha2::Sha384::digest(self.caliptra_rom).into(),
...
pub struct MailboxRecvTxn<'a, TModel: McuHwModel> {
    model: &'a mut TModel,
    pub req: MailboxRequest,
}
impl<'a, Model: McuHwModel> MailboxRecvTxn<'a, Model> {
...
pub trait McuHwModel: SocManager {
    type TBus<'a>: Bus
    where
        Self: 'a;
    ...
    where
        Self: Sized,
    {
        McuHwModel::init_fuses(self, &boot_params.fuses);
    ...
    fn warm_reset_flow(&mut self, fuses: &Fuses) {
        ...
        McuHwModel::init_fuses(self, fuses);
        self.soc_ifc().cptra_bootfsm_go().write(|w| w.go(true));
    }
...
mod tests {
    use crate::{
        mmio::Rv32GenMmio, BootParams, DefaultHwModel, InitParams, McuHwModel, ModelError,
        ShaAccMode,
    };
