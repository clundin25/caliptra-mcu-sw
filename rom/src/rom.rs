...
impl Soc {
    ...
    pub fn set_ocp_lock_fuses(&self, otp: &Otp) -> HekState {
        ...
        let mut hek_state = HekState {
            active_slot: HEK_OFFSETS.len(),
            ...
            .unwrap_or_else(|_| fatal_error(McuError::ROM_OTP_READ_ERROR))
        {
            ...
            hek_state.active_slot = 0;
            hek_state.active_state = HekSeedState::Permanent;
            return hek_state;
        }
        ...
        if HEK_OFFSETS.iter().all(|&offset| {
            ...
            hek_state.active_state = HekSeedState::Unused;
        }
        ...
        if let Some(hek_slot) = hek_slot {
            ...
            hek_state.active_state = HekSeedState::Programmed;
            // TODO(clundin): Set a real active slot.
            // https://github.com/chipsalliance/caliptra-mcu-sw/issues/1071
            hek_state.active_slot = 0;
        } else {
            hek_state.active_state = HekSeedState::Sanitized;
        }
