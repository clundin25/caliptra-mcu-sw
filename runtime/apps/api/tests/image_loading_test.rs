...
mod tests {
    ...
    fn test_load_image1_authorized() {
        ...
        let image_loader: ImageLoaderAPI<Syscalls> = ImageLoaderAPI::new(ImageSource::Flash);
    ...
    static DESCRIPTOR: std::sync::LazyLock<[Descriptor; 1]> =
        std::sync::LazyLock::new(|| [Descriptor::new_empty()]);
    #[test]
    fn test_load_streaming_boot() {
        // Create a fake mailbox driver
        let mailbox_driver = FakeMailboxDriver::new();
        let dma_driver = FakeDMADriver::new();
        let flash_driver = FakeFlashDriver::new();
        ...
        let image_loader: ImageLoaderAPI<Syscalls> =
            ImageLoaderAPI::new(ImageSource::Pldm(&*DESCRIPTOR));
