...
impl DeviceCertsManager {
    ...
    pub async fn get_certificate_chain_digest<S: Syscalls>(
        &self,
        slot_id: u8,
...
mod test {
    ...
    fn test_get_certificate_chain_data() {
        let mut cert_chain_data = SpdmCertChainData::default();
        let device_certs_mgr = DeviceCertsManager;
        let slot_id = 0;
