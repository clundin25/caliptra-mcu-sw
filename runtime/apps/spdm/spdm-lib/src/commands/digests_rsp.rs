...
pub(crate) async fn handle_digests<'a, S: Syscalls>(
    ...
    fill_digests_response(ctx, req_payload).await?;
...
async fn fill_digests_response<'a, S: Syscalls>(
    ctx: &mut SpdmContext<'a, S>,
    ...
    // No slots provisioned or mismatched slot mask
    if provisioned_mask == 0 || provisioned_mask != supported_mask {
    ...
    // Fill Fixed fields
    let dgst_resp_common = GetDigestsRespCommon {
        supported_slot_mask: supported_mask,
        provisioned_slot_mask: provisioned_mask,
    };
    total_payload_len = dgst_resp_common
        .encode(rsp)
        .map_err(|_| ctx.generate_error_response(rsp, ErrorCode::InvalidRequest, 0, None))?;

    // Fill the digests in the response for supported slot mask
    for slot_id in 0..SPDM_MAX_CERT_CHAIN_SLOTS {
        if supported_mask & (1 << slot_id) != 0 {
            // make space for the digest in the buffer
            rsp.put_data(hash_size as usize)
                .map_err(|_| (false, CommandError::BufferTooSmall))?;

            total_payload_len += hash_size as usize;

            let digest_buf = rsp
                .data_mut(hash_size as usize)
                .map_err(|_| (false, CommandError::BufferTooSmall))?;

            ctx.device_certs_manager
                .cert_chain_digest::<S>(slot_id as u8, hash_algo, digest_buf)
                .await
                .map_err(|_| ctx.generate_error_response(rsp, ErrorCode::Unspecified, 0, None))?;

            // pull the data pointer down by the hash size
            rsp.pull_data(hash_size as usize)
                .map_err(|_| (false, CommandError::BufferTooSmall))?;
        }
    }

    // TODO: Fill KeyPairId, CertInfo, KeyUsageBitMask if available

    // Push data offset up by total payload length
    rsp.push_data(total_payload_len)
        .map_err(|_| (false, CommandError::BufferTooSmall))?;
