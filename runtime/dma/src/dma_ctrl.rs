...
use registers_generated::dma_ctrl::{
    bits::*,
    regs::*,
    DMA_CTRL_ADDR,
};
...
impl<'a> EmulatedDmaCtrl<'a> {
    ...
    pub fn handle_interrupt(&self) {
        println!("Device interrupt handler called");
        ...
        if dmactrl_intr.is_set(DmaInterruptState::Error) {
            ...
            println!("Device error interrupt handler called");
        ...
        if dmactrl_intr.is_set(DmaInterruptState::Event) {
            println!("Device event interrupt handler called");
            // Clear the op_status register
            self.registers.dma_op_status.modify(DmaOpStatus::Done::CLEAR);
            ...
            self.dma_client.map(move |client| {
                println!("Device transfer complete");
...
impl crate::hil::DMA for EmulatedDmaCtrl<'_> {
    fn configure_transfer(
        ...
        println!("Device configure_transfer called");
        ...
        self.registers.source_addr_lower.set(src_addr.unwrap() as u32);
        self.registers.source_addr_high.set((src_addr.unwrap() >> 32) as u32);
        self.registers.dest_addr_lower.set(dest_addr.unwrap() as u32);
        self.registers.dest_addr_high.set((dest_addr.unwrap() >> 32) as u32);
    ...
    fn start_transfer(&self, read_route: crate::hil::DmaRoute, write_route: crate::hil::DmaRoute, _fixed_addr: bool) -> Result<(), ErrorCode> {
