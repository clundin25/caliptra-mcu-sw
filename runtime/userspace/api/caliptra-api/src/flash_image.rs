...
impl<'a> FlashLayout<'a> {
    pub fn parse(mut flash: &'a [u8]) -> Result<Self, ErrorCode> {
        ...
        let (checksums, rest) =
            FlashChecksums::ref_from_prefix(flash).map_err(|_| ErrorCode::Fail)?;
        flash = rest;

        let image_count = header.image_count as usize;
        let (image_infos, _) =
            ImageHeader::slice_from_prefix(flash, image_count).ok_or(ErrorCode::Fail)?;
