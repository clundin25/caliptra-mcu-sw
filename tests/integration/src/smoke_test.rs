...
use caliptra_test::{derive, redact_cert, RedactOpts, UnwrapSingle};
use mcu_hw_model::{BootParams, InitParams, McuHwModel, SecurityState};
use openssl::nid::Nid;
...
fn test_rt_wdt_timeout() {
    ...
    let security_state = *mcu_hw_model::SecurityState::default().set_debug_locked(false);
    let init_params = mcu_hw_model::InitParams {
        caliptra_rom: &rom,
        security_state,
    ...
    let mut hw = mcu_hw_model::new(init_params, BootParams::default()).unwrap();
    ...
    let security_state = *mcu_hw_model::SecurityState::default().set_debug_locked(true);
    let init_params = mcu_hw_model::InitParams {
        caliptra_rom: &rom,
        security_state,
        itrng_nibbles: Box::new(RandomNibbles(StdRng::seed_from_u64(0))),
    ...
    let boot_params = mcu_hw_model::BootParams {
        wdt_timeout_cycles: rt_wdt_timeout_cycles,
        ..Default::default()
    };

    let mut hw = crate::run_test(None, None, Some(init_params), Some(boot_params));
